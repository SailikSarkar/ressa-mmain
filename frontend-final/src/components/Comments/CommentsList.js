...
export default function CommentsList({ comments }) {
  ...
  return (
    <div>
      <ul className="divide-y bg-white dark:bg-gray-800 w-96 divide-gray-200 rounded-2xl shadow-2xl p-3 mt-5">
        <div className="text-gray-400">{comments?.length} Comments</div>
        <>
          {comments?.length <= 0 ? (
            <h1 className="text-red-400 text-lg font-mono tracking-widest text-center">No comments</h1>
          ) : (
